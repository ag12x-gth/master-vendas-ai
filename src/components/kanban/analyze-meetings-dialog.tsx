          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="bg-blue-50 dark:bg-blue-950 p-4 rounded-lg">
                <p className="text-xs text-muted-foreground mb-1">Conversas Analisadas</p>
                <p className="text-2xl font-bold">{result.stats.conversationsAnalyzed}</p>
              </div>
              <div className="bg-green-50 dark:bg-green-950 p-4 rounded-lg">
                <p className="text-xs text-muted-foreground mb-1">Agendamentos Detectados</p>
                <p className="text-2xl font-bold">{result.stats.meetingsDetected}</p>
              </div>
              <div className="bg-purple-50 dark:bg-purple-950 p-4 rounded-lg">
                <p className="text-xs text-muted-foreground mb-1">Leads Movidos</p>
                <p className="text-2xl font-bold">{result.stats.leadsMoved}</p>
              </div>
              <div className="bg-orange-50 dark:bg-orange-950 p-4 rounded-lg">
                <p className="text-xs text-muted-foreground mb-1">Já no Estágio Correto</p>
                ...
                <ScrollArea className="h-[250px] rounded-md border p-4">
                  <div className="space-y-2 pr-4">
                    {result.details.map((detail, index) => (
                      <div
                        key={index}
                        className="flex items-start gap-3 text-sm p-3 rounded-md hover:bg-muted transition-colors"
                      >
                        {detail.moved ? (
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-medium break-words">{detail.contactName}</p>
